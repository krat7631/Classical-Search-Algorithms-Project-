...
class State:
    ...
    def __iter__(self) -> Iterator[int]:
        """Allows unpacking: row, col = state."""
        yield self.row
        yield self.col
...
class SearchResult:
    ...
    # when solution_found is False: time_budget, expansion_limit, depth_limit, or exhausted
...
class GridWorld:
    ...
    def get_successors(self, state: State) -> List[Tuple[State, float]]:
        """(successor, cost) for valid 4-neighbour moves. Cost = 1."""
        successors = []
...
def build_path(parent: dict, state: State) -> List[State]:
    """Reconstruct path from goal to start using parent pointers. state is the goal (or current) node; parent[state] is the state we expanded from. Walk backwards then reverse to get start→goal order."""
    path = []
    current = state
...
def generate_grid_world(
    ...
    start = (0, 0)
    goal = (rows - 1, cols - 1)
    # Start and goal must stay free so we only place obstacles on other cells
    protected = {start, goal}

    # Retry until we get a grid where goal is reachable from start (BFS connectivity check)
